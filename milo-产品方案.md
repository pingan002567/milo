...
> 定位：DeerFlow 生态的 **Agent 应用商店 + AI 组织指挥台**
> 一句话：下载（招募）或生成调教好的 Agent 成员，组建你的 AI 组织，编辑组织架构，统一指挥干活。
> 市场话术：**你的一人公司**——招聘数字员工，一个人指挥一支随叫随到的班子（展示层隐喻，领域模型保持通用组织，见 §2.5）。
...
```
调教 (llm-space)  →  [ 分发：空白 ]  →  运行 (DeerFlow 2.0)  →  [ 多实例编排：空白 ]
                      ↑ Milo Registry               ↑ Milo 秘书长
```

字节已做好：运行时、调教工具、包格式雏形（SKILL.md）。缺的两层正是本产品：

1. **分发层**：SKILL.md 有安装命令但没有托管市场——无版本管理、无评级、无搜索发现、无付费机制。
2. **编排层**：DeerFlow 2.0 是"一个实例一个超级智能体"，子 agent 只在实例内部动态生成；**没有跨实例的多 agent 协同**——不能让"你调教的法务 agent + 我下载的财务 agent + 官方的研究 agent"组队干一个活。
...
| 画像 | 需求 | 对应功能 |
|---|---|---|
| Agent 作者（开发者/深度玩家） | 调教成果想被使用、署名、变现 | 发布、版本管理、下载统计、分成 |
| Agent 使用者（个人/小团队） | 不会调教，想直接用高质量 agent | 市场搜索、评分排序、一键部署 |
| 企业团队 | 多个专业 agent 协同完成业务流程，需私有化 | 组织编排、私有 Registry、审计 |
...
获取 Agent                          组建组织                       指挥干活
┌──────────────────────┐    ┌───────────────────────┐    ┌───────────────────────┐
│ A. 市场下载            │    │ milo deploy            │    │ 自然语言下达任务         │
│   浏览/搜索/看评测报告   │ →  │ 每个 agent 起一个        │ →  │ 秘书长 分解、路由、   │
│ B. 向导生成            │    │ DeerFlow 2.0 实例       │    │ 派单、汇总              │
│   问答式生成 MiloPack   │    │ (容器/K8s)             │    │ 全程 trace 可观测        │
│ C. llm-space 导入      │    │                        │    │                        │
└──────────────────────┘    └───────────────────────┘    └───────────────────────┘
```
...
### 2.5 叙事分层：前端「一人公司」×领域「通用组织」（2026-07-19 决策）

面向市场的第一叙事采用**「一人公司 + 数字员工」**：认知门槛低、有现成的市场心智（企业客户熟悉"数字员工"采购话术），且"一个人指挥一支班子"精准命中个人专业者与小团队的情感诉求。但该隐喻**只存在于展示层与营销话术**：
...
| 层 | 名称 | 落盘 | 归属 | 动作 |
|---|---|---|---|---|
| 发现 | **Agent 市场** | —（远端/本地源） | 公共 | ⭐收藏（记引用）/ 下载 |
| 拥有 | **Agent 库** | `~/.milo/library/<name>@<version>/` | 用户全局（跨公司共享） | 聘用（实例化）/ 移除 |
| 雇佣 | **数字员工（实例）** | org.yaml + `orgs/<org>/members/<实例名>/` | 某个公司 | 入职/停职/复岗/辞退 |

**关键规则：**
- 市场职责收窄为"发现 + 验货"（质检报告、权限前置声明），**不再直接产生雇佣**；
  收藏与下载解耦（收藏只记引用不占磁盘，存 `settings/favorites.yaml`）。
- **聘用必须起名**：实例名公司内唯一（默认建议 `模板名-N`），命名对话框附
  「立即入职」勾选；人事红线不变，三层动作全部仅用户可发起。
- org.yaml 成员记录：`{name: 小张, agent: py-dev@0.1.0, enrolled}`——实例
  **pin 模板版本**，模板升级不影响在岗实例（实例迁移是 v1）。旧格式
  `pack: <路径>` 向后兼容可读，新写入一律模板引用。
- 同模板多实例天然成立（"小张""小李"同为 py-dev 实例），工作区/记忆互不相干。
- 库中模板被实例引用时禁止移除（实例重启需模板源）；**不做实例换模板**——
  换模板=换人，走辞退+新聘，不沿用旧记忆。
- Registry v0（M4）接入后，"下载"获得真实远端语义，接口形状不变
  （v0 下载 = 从本地源目录拷入库）。
...
┌─────────────────────────── Milo 控制面 ────────────────────────────────┐
│                                                                        │
│  Console (Web)          Registry               秘书长               │
│  ├ 市场浏览/搜索          ├ MiloPack 存储/版本    ├ 任务分解 (planner LLM)  │
│  ├ 组织仪表盘            ├ 评测复跑 CI           ├ 能力路由 (按 manifest    │
│  ├ 任务下达/trace 回放    ├ SkillScan/安全审查    │   capabilities 匹配)    │
│  └ Agent 生成向导        └ 评分/下载统计          ├ 派单/重试/超时           │
│                                                 └ 结果聚合/仲裁           │
│  CLI: milo publish / install / deploy / run / logs                     │
│  统一鉴权层 (API key / OIDC) ← DeerFlow 官方无鉴权，必须由我们补            │
└───────────────┬────────────────────────────────────────────────────────┘
                │ 派单/回收：DeerFlow 2.0 HTTP API + Redis stream bridge
┌───────────────┴──────────────── 执行面（组织）───────────────────────────┐
│  ┌─ Pod: 法务 Agent ─┐  ┌─ Pod: 财务 Agent ─┐  ┌─ Pod: 研究 Agent ─┐      │
│  │ DeerFlow 2.0 实例 │  │ DeerFlow 2.0 实例 │  │ DeerFlow 2.0 实例  │      │
│  │ + MiloPack 注入   │  │ + MiloPack 注入   │  │ + MiloPack 注入    │      │
│  │ + 沙箱 (官方K8s)  │  │                   │  │                    │      │
│  └──────────────────┘  └───────────────────┘  └────────────────────┘      │
│  共享层：Postgres (状态/审计)  Redis (消息)  对象存储 (工件/mnt/user-data)   │
└──────────────────────────────────────────────────────────────────────────┘
```
...
- 成员 = DeerFlow 实例 + 注入的 MiloPack（档案与技能证书）。部署（加入）时：拉取 MiloPack → 渲染出实例的 skills 目录、MCP 配置、system prompt → 起容器（复用官方 AIO 容器镜像与 K8s 沙箱支持）。
- **非目标（明确排除）**：实例内部的 Lead Agent、动态 subagent、18 个中间件是成员的"个人工作方法"，由 DeerFlow 自治，Milo 不感知、不编排、不配置。这把 Milo 与 DeerFlow 快速迭代（一版本 180 PR）的耦合面收敛到 SDK 接口一层。
- **Member Contract（Milo 与成员之间的唯一接口，六个动作）**：
  ...
  合同由 MemberAdapter 适配层实现，底层调用官方 Python SDK（DeerFlowClient）；DeerFlow 升级只改适配层。
- Trace 分层：Milo 只存成员级汇报与交付记录；实例内部 trace 链接到 DeerFlow 自带观测（Langfuse/Monocle），不复制、不解析。
- 隔离即安全边界：官方 Python REPL 无沙箱、API 无鉴权，通过"每成员一容器 + 网络策略按 manifest.permissions 收敛 + 控制面统一鉴权"解决。
...
### Phase 1（0–3 个月）：格式 + CLI + Registry —— 先让 agent 能流通
- MiloPack v1 规范 + 校验器
- `milo publish / install / import(llm-space) / run`（单 agent 本地跑通：起一个 DeerFlow 2.0 容器并注入包）
- Registry 最小版：存储、版本、搜索、SkillScan 接入、评测复跑 CI
- 种子内容：官方 skills 转制 + 自制 10–20 个高质量包（冷启动靠自营内容）
- **验证指标**：外部作者主动发布的包数量；install 后 7 日留存

### Phase 2（3–6 个月）：秘书长 + Console —— 让 agent 能组队
- **小组模式**部署（单层编制：组长 → 秘书长 → 组员；K8s operator 或 docker-compose 两档）
- 内置能力自动路由 + 失败重试（自定义 CEL 路由、DAG 流水线、部门封装延至 Phase 3）
- 桌面端 App：市场、小组仪表盘、任务 trace 回放（对标 llm-space 的 trace 体验但面向多 agent）
- 统一鉴权 + 审计日志（企业刚需）
- 指挥体系落地（第五章）：秘书长 单一入口、双通道汇报、action guards、计划前置授权
- **验证指标**：多 agent 任务占比；单任务平均成员数；单任务对用户平均打扰次数；override 率；待批队列时长

### Phase 3（6–12 个月）：市场化运营
- 评分/评论/榜单、付费包与分成、组织私有 Registry（企业版收费点）
- 编制进阶：自定义路由 + DAG 流水线 → 部门封装 + 主管成员 + replicas 工位池
- 竞争/评审协同模式；组织模板（"投研部""合规部"成建制分发）
- IM 深度集成：组织直接进飞书/微信群接单（复用 DeerFlow 渠道网关）
...
| 风险 | 评估 | 对策 |
|---|---|---|
| DeerFlow 2.0 迭代过快（一版本 180 PR），API/skills 格式变动 | 高 | manifest 里 `deerflow.minVersion` + Registry 按运行时版本做兼容矩阵；MiloPack 只聚合官方概念不魔改内部 |
| 字节官方下场做市场 | 中 | 差异化押"评测质检 + 组织编排 + 企业私有化"；官方即使做市场也大概率不做跨实例编排 |
| 安全：恶意包（prompt 注入、危险 skill、MCP 供应链） | 高 | 上架三重审查（SkillScan + 注入检测 + permissions 审查）+ 运行期容器/网络隔离 + 包签名 |
| llm-space 格式变动（v4 迭代快、闭环开发） | 中 | 导入层做版本适配器；evals 内部存我们自己的规范化格式，Thread JSON 只是导入源 |
| 冷启动（双边市场） | 高 | 先做单边价值：即使不下载别人的包，"生成向导 + 组织编排"对自用也成立；自营种子内容 |
| 1.x 用户迁移预期 | 低 | 明确只支持 2.0；提供 1.x prompt → MiloPack 的一次性迁移脚本作引流 |
