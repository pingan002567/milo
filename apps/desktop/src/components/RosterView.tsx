...
/** 名册页：org.yaml 的可视化（spec = 期望名册，由你签署；秘书只执行）。 */
export function RosterView({ org }: { org: string }) {
  ...
  if (!data) return <div className="muted">读取名册中…</div>;

  return (
    <>
      <div className="h">名册 · org.yaml</div>
      <div className="muted" style={{ maxWidth: 620, marginBottom: 12 }}>
        名册是团队的期望状态，由你签署；秘书按它执行加入/移出，无权自行变动。
        ...
        <div key={m.name} className="card" style={{ padding: "12px 16px", marginBottom: 10, maxWidth: 760 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            ...
            <span className={`chip ${m.enrolled === false ? "warn" : m.loaded ? "ok" : ""}`}>
              {m.enrolled === false ? (m.has_workspace ? "停职中" : "待加入")
                : m.loaded ? (m.busy ? "在岗 · 工作中" : "在岗") : "未运行"}
            </span>
      ...
      <div className="card" style={{ padding: "12px 16px", maxWidth: 760 }}>
        <dl className="ikv">
          <dt>并行成员上限</dt><dd>{data.limits.maxParallelMembers ?? 5}（监督幅度证据：人有效监督约 5 个对象）</dd>
          <dt>待批队列背压</dt><dd>{data.limits.pendingQueueBackpressure ?? 3}</dd>
        </dl>
        <div className="muted" style={{ fontSize: 11.5 }}>
          小队模式（v0）：单层结构 · 成员间无直连 · 产物经 artifact 交接。
          部门与自定义路由为后续版本。
        </div>
      </div>
    </>
